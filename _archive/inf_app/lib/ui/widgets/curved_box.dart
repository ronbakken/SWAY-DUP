...
class _CurvedBoxClipper extends CustomClipper<Path> {
  ...
  bool shouldReclip(_CurvedBoxClipper oldClipper) =>
      oldClipper.curveFactor != curveFactor || oldClipper.top != top || oldClipper.bottom != bottom;
}
...
Path _createCurvedPath(double curveFactor, Size size, bool top, bool bottom) {
  ...
  if (top && !bottom) {
    ...
    // bottom only
    final rect = Rect.fromLTRB(-curve, -size.height, size.width + curve, size.height);
    return Path()..addArc(rect, 0.0, pi);
  } else {
    // both top and bottom
    final rect = Rect.fromLTRB(-curve * 0.5, 0, size.width + curve * 0.5, size.height);
