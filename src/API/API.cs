                ...
                new ServiceInstanceListener(
                    serviceContext =>
                        new KestrelCommunicationListener(
                            ...
                            (url, listener) =>
                            {
                                ServiceEventSource.Current.ServiceMessage(serviceContext, $"Starting Kestrel (HTTP) on {url}");

                                return new WebHostBuilder()
                                            .UseKestrel(
                                                opt =>
                                                {
                                                    var httpPort = serviceContext.CodePackageActivationContext.GetEndpoint("HttpServiceEndpoint").Port;
                                                    opt.Listen(IPAddress.IPv6Any, httpPort);
                                                })
